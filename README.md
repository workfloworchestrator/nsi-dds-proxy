...
| `OIDC_REQUIRED_GROUPS` | `[]` | Groups required for OIDC-authenticated access. As an env var, accepts comma-separated (`g1,g2`), a single value (`g1`), a JSON array (`["g1","g2"]`), or empty (treated as no groups). Use `[]` or leave empty for no group check (any authenticated user is allowed). Matched against the parsed `X-Auth-Request-Groups` header (comma- or whitespace-separated). |
